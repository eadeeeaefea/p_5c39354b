    ...
    while (true) {
        try {
            ...
            if (true) {
                ...
                if (frame_count > history_num_) { //取完背景提取所需帧数（前景提取稳定）后进行后续处理
                    Mat frame_show = current_frame_.clone();
                    if (check_num_ < check_thres_) {
                        armor_checker.run(current_frame_, robot_fgmask, enemy_rois,
                                          ENEMY_COLOR); //输入fgmask图片，输出ROI矩形框,分辨机器人颜色,在ROI区域中查找装甲板并输出装甲板ROI
//TODO: move it inside class
#ifdef SHOW_IMAGE
                        //画出方机器人的位置
                        for (int i = 0; i < enemy_rois.size(); i++) {
                            if (ENEMY_COLOR) { //画出对应颜色机器人
