...
/*
    ...
    pack[10] --> tail = 0xE4
*/
    while (!(read_bytes[0] == 0x31) && !(read_bytes[10] == 0xE4))
    {
        for (int i = 0; i < 10; ++i)  read_bytes[i] = read_bytes[i+1];
        if (::read(fd_, &read_bytes[10], 1) == 1)
        {
...
//            throw SerialException("Read data failed.");
    ...
    uint8_t check_sum = static_cast<uint8_t>(read_bytes[1] + read_bytes[2] +
                                             ...
                                             read_bytes[7] + read_bytes[8]);

    if (check_sum == read_bytes[9])
    {
        read_pack.plot_type = static_cast<int>(read_bytes[1]);
        read_pack.curve_num = static_cast<int>(read_bytes[2]);
        int16_t temp_value;
        temp_value = (static_cast<int16_t>(read_bytes[4]) << 8) + static_cast<int16_t>(read_bytes[3]);
        read_pack.plot_value[0] = static_cast<double>(temp_value) * 0.01;
        temp_value = (static_cast<int16_t>(read_bytes[6]) << 8) + static_cast<int16_t>(read_bytes[5]);
        read_pack.plot_value[1] = static_cast<double>(temp_value) * 0.01;
        temp_value = (static_cast<int16_t>(read_bytes[8]) << 8) + static_cast<int16_t>(read_bytes[7]);
        read_pack.plot_value[2] = static_cast<double>(temp_value) * 0.01;
