...
void TargetSolver::run(const RotatedRect &armor, Target &target)
{
    ...
    bool is_big_armor = 0;
    double ratio = (armor.size.width > armor.size.height) ? (armor.size.width / armor.size.height) : (armor.size.height / armor.size.width);
                                       ...
                                       Target &ptz_position)
{
    ...
    ptz_position.y = (camera_position.at<double>(1, 0) - 59.51) / 1000;
    ptz_position.z = (camera_position.at<double>(2, 0) + 37.76) / 1000;
}
