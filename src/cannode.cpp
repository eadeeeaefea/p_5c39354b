...
bool CanNode::init()
{
    ...
    {
        std::cerr << dev_name_ << " socket error.\n";
        status = false;
    }
    strcpy(ifr_.ifr_name, dev_name_.c_str());

    if (ioctl(skt_, SIOCGIFINDEX, &ifr_) < 0)
    {
        std::cerr << dev_name_ << " ioctl error.\n";
        status = false;
    }
    ...
    if (setsockopt(skt_, SOL_CAN_RAW, CAN_RAW_FILTER, &cfilter_, sizeof(cfilter_)) != 0)
    {
        ...
        std::cout << dev_name_ << " sockopt error.\n";
        status = false;
    }
    mode_ = Mode::STAND_BY;
...
bool CanNode::send(const SendPack &send_pack)
{
    ...
    */
    ...
    frame.data[1] = static_cast<uint8_t>(send_pack.mode);
    int16_t *data_ptr = reinterpret_cast<int16_t *>(frame.data + 2);
...
{
    if ((frame.data[0] == 0xAA) && (frame.data[7] == 0x55))
    {
        read_pack.mode = frame.data[1] > 10;
        read_pack.enemy_color = (frame.data[1] % 10) == 0 ? Mode::ARMOR : Mode::RUNE;
        int16_t temp_pitch = (static_cast<int16_t>(frame.data[2]) << 8) +
                             static_cast<int16_t>(frame.data[3]);
        int16_t temp_yaw = (static_cast<int16_t>(frame.data[4]) << 8) +
        ...
        read_pack.bullet_speed = static_cast<double>(frame.data[6]);
