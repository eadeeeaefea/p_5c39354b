...
void HPredictor::init() {}

void HPredictor::run(double &x, double &y, double &z, double ptz_pitch,
                    double ptz_yaw) {
...
void HPredictor::motion_prediction(double &x, double &y, double &z,
  ...
  if (object_coordinate.size() < 2) {
    ...
    if (dis < 0.25 && dis > 0.01 && judgement()) {
    }
  }
  x = predict_object_motion.x;
  y = predict_object_motion.y;
  z = predict_object_motion.z;
  anti_coordinate_transformation(x, y, z, angle_pitch, angle_yaw);
}

void HPredictor::explosive_motion_prediction(double &x, double &y, double &z,
                                            double ptz_pitch, double ptz_yaw) {
  double angle_pitch, angle_yaw;
  angle_pitch = ptz_pitch * PI / 180;
  angle_yaw = ptz_yaw * PI / 180;
  ...
  if (object_coordinate.size() < 2) {
    ...
    dis = point_distance(object_coordinate[0], object_coordinate[1]);
    if (dis < 0.25 && dis > 0.01 && judgement()) {
      object_accurate_speed =
          (object_coordinate[1] - object_coordinate[0]) / time;
...
void HPredictor::normal_motion_prediction(double &x, double &y, double &z,
                                         double ptz_pitch, double ptz_yaw) {
  double angle_pitch, angle_yaw;
  angle_pitch = ptz_pitch * PI / 180;
  angle_yaw = ptz_yaw * PI / 180;
  ...
  if (object_coordinate.size() < 10) {
    ...
    if (dis < 0.25 && dis > 0.02 && judgement()) {
      for (c = 0; c <= 4; c++) {
        object_inaccurate_speed[c] =
            (object_coordinate[c + 5] - object_coordinate[c]) / time;
      }
      object_accurate_speed =
          (object_inaccurate_speed[0] + object_inaccurate_speed[1] +
           object_inaccurate_speed[2] + object_inaccurate_speed[3] +
           object_inaccurate_speed[4]) /
          5;
...
void HPredictor::coordinate_transformation(double &x, double &y, double &z,
                                          double ptz_pitch, double ptz_yaw) {
  double absolute_x;
  double absolute_y;
  double absolute_z;
...
void HPredictor::anti_coordinate_transformation(double &x, double &y, double &z,
                                               double ptz_pitch,
                                               double ptz_yaw) {
  double objective_x;
  double objective_y;
...
double HPredictor::point_distance(Point3d point1, Point3d point2) {
  double distance;
...
void HPredictor::get_excercise_time(double x, double y, double v,
                                   double ptz_pitch) {
  double time_square;
